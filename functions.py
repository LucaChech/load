        ...
        for pic in block:
            ...
            for frameN in range(n_frames):
                if 'tone1' in my_dict[pic[1:-1]] and frameN == start_frame :
                    ...
                    tone_trial_number = trial_number
...
#            print T
            ...
            if i_counter in probes_position_list[probes_position_index]:
                # If there is a visual search probe
                print 'pic',pic
                ...
                if test:
                    ...
                    keys = event.waitKeys(keyList=['q','p','space'])
                    key_time_since_question = question_timer.getTime()
                    key_time_since_tone = tone_timer.getTime()
                rt_space = -999
                wait = True
                for k in keys:
                    if k[0] == 'r':
                    ...
                    if k[0] == 'space':
                        rt_space = [k[0],k[1]]
                        if waiting_for_spacebar:
                            experiment_details[tone_trial_number]['RT_TO'] = key_time_since_tone
                        else:
                            trial_details['RT_TO'] = key_time_since_tone
                        waiting_for_spacebar = False
                        images_waited = 0
                        
                    if k[0] == 'q' or k[0] == 'p':
                        wait = False                       
                        rt_vs = key_time_since_question
                while wait:
                    ...
                    if ('p' in letters) or ('q' in letters):
                        wait = False
                        rt_vs = key_time_since_question
                #ASSUMPTION: keys is definitely q or p, space cannot be in there
                my_dict[pic[1:-1]].append(keys[0][0])
                my_dict[pic[1:-1]].append(str(keys[0][1]))
                my_dict[pic[1:-1]].append(rt_space) #now wrong
