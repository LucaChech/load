...
#Adding critical (no probe) trials to the block
    def constrained_sum_sample_pos_adding_criticals(n, total):
        ...
        global block, critical_per_block, new_list, diff
        while True:
            ...
            diff = []
            for i in range(9 - critical_probes_counter - 1):
                diff.append(new_list[i+1] - new_list[i])
            if 1 not in diff:
                block = normal_per_block
                #print len(block)
                random.shuffle(critical_per_block)
                for i in range(len(critical_per_block)):
                    block.insert(new_list[i], critical_per_block[i])
                return  block, new_list
                break
#-----------------------------------------------------------------------------
#Adding probe trials to the block
    def constrained_sum_sample_adding_probes(n, total):
        """Return a randomly chosen list of n positive integers summing to total.
        Each such list is equally likely to occur."""
        global probes_per_block, new_list
        global sequences_length
        dividers = sorted(random.sample(xrange(1, total), n - 1))
        sequences_length = [a - b for a, b in zip(dividers + [total], [0] + dividers)]
        acc = 0
        ...
        for i in sequences_length:
            acc = acc + i
            new_list.append(acc - 1)
        random.shuffle(probes_per_block)
        for i in range(len(probes_per_block)):
            block.insert(new_list[i], probes_per_block[i])
        return  block, new_list
#-----------------------------------------------------------------------------    
    constrained_sum_sample_pos_adding_criticals(9 - critical_probes_counter,54)
    print len(block)
    #print new_list
    #for n in range(len(new_list)):
    #    print new_list[n], my_dict[block[new_list[n]]]
    #print diff
    constrained_sum_sample_adding_probes(6,60)
    print len(block)
    #print new_list
    #for n in range(len(new_list)):
    #    print new_list[n], my_dict[block[new_list[n]]]
...
#-------------------------------------------------------------------
        waiting_list = [i for i in range(60,0,-1)]
