...
def process_observer(obs):  
    f = '../output/new_participants/participant_'+obs+'.pik'
    import pickle
    with open(f,'r') as of:
        D = pickle.load(of)
...
def process(df):
    ...
    for i in range_from_1(si):
        if not pd.isnull(df.loc[i,'RT_TO']) and df.loc[i,'tone_sdt'] == 'HI':
    ...
    print 'negative RT_TO values: ', df_negative.shape
    df_negative = df[(df['RT_VS'] < 0)]
    print 'negative RT_VS values: ', df_negative.shape
    ...
    for i in range_from_1(si):
        ...
        no_vis_target_or_right = (df.loc[i,'present_absent'] == 'No Target')# or df.loc[i,'correct_vs'] == 1)
    ...
    df_full = df[(df['trial_type']=='Critical')&(df['present_absent']=='No Target')] 
    print 'a',df_full.shape[0]
    df_slim = df[(df['trial_type']=='Critical')] 
    print 'b',df_slim.shape[0]
